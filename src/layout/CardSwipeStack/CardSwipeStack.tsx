...
function CardSwipeStack(){
    ...
    const mouseMove = useCallback((e: React.MouseEvent<HTMLDivElement, MouseEvent> | null, t:React.TouchEvent<HTMLDivElement> | null) => {
        if(isMouseDown){
            ...
            //getting all "decision container" elements. In this case - Red/Green containers.
            if(!decisionContainers){
                let elements = document.querySelectorAll('.decisionContainer');
                let elementArray:Element[] = [];
                elements.forEach(element => {
                    elementArray.push(element);
                })
                setDecisionContainers(elementArray);
            }
            

            // Detecting if mouse is over the container, and setting the dragAction state to element id
            let elementHoverFound = false;
            if(decisionContainers){
                decisionContainers.forEach(element => {
                    let elementBounds = element.getBoundingClientRect();
                    if(cursorPosX > elementBounds.left && cursorPosX < elementBounds.right && cursorPosY > elementBounds.top && cursorPosY < elementBounds.bottom){
                        let elementID = element.getAttribute('id');
                        elementHoverFound = true;
                        if(dragAction !== elementID){
                            setDragAction(elementID);
                        }
                    }
                });
            }
            if(!elementHoverFound){
                setDragAction("");
            }
    ...
    return(
        ...
        >
            <NoContainer dragOver={dragAction === "no"} className="decisionContainer" id="no" ref={noRef}>
